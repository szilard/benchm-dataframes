...
experience, if you are using R, I would recommend data.table (also because of the concise syntax and numerous features). If you use python, polars is a good choice (faster than pandas and also a somwhat nicer syntax). If you prefer to write SQL, duckdb is a great embedded analytical database engine (virtually zero setup) that can be used from both R or python. If you also don't mind the extra work in setup and maintenance of a database server (and again you like SQL), clickhouse is a great choice. 